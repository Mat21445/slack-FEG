...
func main() {
	...
	for _ = range time.Tick(time.Hour) {
		...
		for i := 0; i < len(web_data.Data.Catalog.SearchStore.Elements); i++ {
			if web_data.Data.Catalog.SearchStore.Elements[i].EffectiveDate < time.Now().UTC().Format("2006-01-02T15:04:05.000Z") && web_data.Data.Catalog.SearchStore.Elements[i].EffectiveDate > TmpTime {

				// send the message using Slack webhook
				attachment1 := slack.Attachment{}
				attachment1.AddField(slack.Field{Title: "New Free Epic Game Available!:", Value: web_data.Data.Catalog.SearchStore.Elements[i].Title}).AddField(slack.Field{Title: "Description:", Value: web_data.Data.Catalog.SearchStore.Elements[i].Title})
				attachment1.AddAction(slack.Action{Type: "button", Text: "Check it!", Url: "https://www.epicgames.com/store/en-US/free-games", Style: "primary"})
